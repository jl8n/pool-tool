...
// You can also define the response structure if needed
interface SetStateResponse {
  // Define your response structure here
}

server.put<{ Body: SetStateBody }>('/setState', async (req, reply) => {
  try {
    const { id, state, equipmentType } = req.body;
    ...
    if (typeof id === 'undefined' || typeof state === 'undefined' || typeof equipmentType === 'undefined') {
      return reply.status(400).send({ error: 'Missing required fields in the request body' });
    }
    ...
    switch (equipmentType) {
      case 'circuit':
        response = await callPoolApi('state/circuit/setState', 'PUT', { id, state });
      ...
      // case 'anotherType':
      //     response = await handleAnotherType(id, state);
      //     break;
    ...
    // Send the response from the API call

  } catch (error) {
    console.error('Error in /setState endpoint:', error);
    reply.status(500).send({ error: 'Internal Server Error' });
  }
});
